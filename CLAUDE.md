# Juniper Flats Pool & Spa — website (demo-service)

This repo IS the website for Juniper Flats Pool & Spa. There is no dashboard and no CMS: you
change the site by editing these files and pushing. Live at https://demo-service.pages.dev/.
...
"Done" means live at https://demo-service.pages.dev/, not edited on disk. Relay sessions run from the
client workspace one level up, so git takes the form
`git -C repos/demo-service …`; never `cd` into the repo first (that is always
blocked). After any requested change, without waiting to be asked:

1. Look at your work: `shot repos/demo-service/<page>.html` (add `--mobile` for
   ...
   run `shot check repos/demo-service/<page>.html`. Fix what's off before you
...
2. Commit on `main` with a short plain-English message
   (`git -C repos/demo-service add -A && git -C repos/demo-service commit -m "…"`).
3. Push: `git -C repos/demo-service push`. The repo is the record.
4. Publish: `site publish demo-service`. It sends exactly what is committed to
...
5. Confirm the live site serves the change, with exactly this shape (no
   pipe, no redirect):
       ...
       curl -s https://demo-service.pages.dev/PAGE.html

   (for the home page, `curl -s https://demo-service.pages.dev/`). Read the output and look for the
...
Undo the last change with `git -C repos/demo-service revert HEAD`, push, and
...
A site gets a database with `site data demo-service` (PLAYBOOK § Data). Then
...
`SITE_ADMIN=1 site domain demo-service example.com` from the workspace (adds
...
client at the end is `site transfer demo-service <their-github-user>`.
